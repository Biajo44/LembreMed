# AlertSaude-

Aplicativo para controle e lembrete de medicamentos.

## 🩺 Descrição
O **AlertSaude** ajuda o usuário a organizar seus medicamentos e não esquecer os horários das doses.  
Com uma interface simples e intuitiva, o app permite cadastrar usuários, realizar o login, cadastro de medicamentos, listagem de medicamento e detalhes.

## 🚀 Funcionalidades
- Cadastro de usuários
- Validação de senha e dados do usuário
- Cadastro de medicamentos  
- Exibição de lista de medicamentos  
- Interface gráfica desenvolvida em Java Swing  

## 🧠 Objetivo
Facilitar o acompanhamento do uso de medicamentos, garantindo que o usuário tenha controle sobre horários, doses e duração do tratamento.

## 🛠️ Tecnologias
- **Java**
- **Swing (interface gráfica)**
- **NetBeans IDE**
- **GitHub** para versionamento

## 📦 Como executar
1. Clone este repositório:
   ```bash
   https://github.com/Biajo44/AlertSaude-.git
   

Projeto desenvolvindo por Joelma Beatriz 👩‍💻
Estudante de analise e desenvolvimento de sistema
